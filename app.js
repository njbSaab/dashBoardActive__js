...
class DashboardItem {
  ...
  changeView(view){
      ...
      this.prev.innerText = `Lasr ${DashboardItem.PERIODS[this.view]} - ${previous}hrs`
...
document.addEventListener('DOMContentLoaded', () =>{
    getDashBoardData()
    .then(data => {
        const activities = data.map(activ => new DashboardItem(activ))
        ...
        selectors.forEach(selector => {
            selector.addEventListener('click', function(){
                ...
                const currentView = selector.innerHTML.trim().toLowerCase()
                activities.forEach(activ => activ.changeView(currentView))
            })
        })
    })
})
